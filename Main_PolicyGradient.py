...
import testModel_PolicyGradient_lr
...
#        num_inter = 149                  #
##        self.conv = nn.Conv2d(num_inter+num_select, num_inter, kernel_size=1, stride=1, bias=False)
#        self.conv = nn.Conv2d(184, num_inter, kernel_size=1, stride=1, bias=False)   #!!!!!!!!!!!!!!!别的站点需要调参数
#
...
#
##        x = self.conv(x.view(-1,num_inter+num_select,10,1))
#        x = self.conv(x.view(-1,184,10,1))   #别的站点需要调参数
#
##        print(x.size())
#
#        x = x.view(-1,num_inter,10)     #!!!!!!!!!!!!!!!!!别的站点需要调参数
##        print(x.size())
#        
#        x = self.fc1(x)
##        print(x.size())
...
def cal_reward_lr_single_station(t_h,t_r,t_u,select_station,select_inter,s_b,stationFlag_test):
    s_inter = np.where(s_b[0,:,0] == 1)
    s_inter = np.array(s_inter).tolist()[0]
    select_inter = np.array(select_inter).reshape(-1,1)
    select_station = np.array(select_station).reshape(-1,1)
...
def cal_reward_lr_station(t_h,t_r,t_u,select_station,select_inter,s_b,stationFlag_test):
    s_inter = np.where(s_b[0,:,0] == 1)
    s_inter = np.array(s_inter).tolist()[0]
    select_inter = np.array(select_inter).reshape(-1,1)
    select_station = np.array(select_station).reshape(-1,1)
...
#                sta = np.where(station_flag_lr==1)
    ...
    t_p_lr_real = t_p_lr_real[:,:,sta_h]
    #D-value
    abx = np.abs(t_p_lr_real - t_r_lr_real)
    varx = np.var(abx)
    abx = np.mean(abx)
#    x = abx/varx
#    loss = 1/math.log(abx+1)
#    print(x)
    loss = 1/abx*10
#    print(loss)
...
if __name__ == "__main__":
    # 1. 加载数据
#    Line = "110"     #num_inter = 149

    Line = 60
    OD = 0                        #输入data是否增加OD数据，0为不增加OD数据，1为增加OD数据;可以增加OD数据的线路号为2、14、60、74、82、110、121、421、498,其他线路没有增加OD数据
    bound = 0
...
#    data_path="./data_110/data_target.mat"
#    test_path="./data_110/test_data.mat"
##    testData = './data_2/ test_date11.mat'
#    historyTime_path = "./data_110/historyTime_110.mat"

    
    data_train = scio.loadmat(data_path)
    data_test = scio.loadmat(test_path)
#    testData = scio.loadmat(testData)
...
#    oldGradient = data_train.get('oldGradient')
    ...
    if OD == 1:            #增加OD数据
        ...
        mean_startTime = data_train.get('mean_startTime')
        maxStartTime = data_train.get('maxStartTime')
        stationFlag_test = data_train.get('stationFlag_test')
        LocSEStop = data_train.get('LocSEStop')
        ...
        data = (data[perm_train,:,:]).reshape((-1,10,predictionLength) ,order="F")
...
#        net = Net().cuda()
    ...
    if OD == 0:           #不增加OD数据
        ...
        data = (data[perm_train,:,:]).reshape((-1,8,predictionLength) ,order="F")
        target = (target[perm_train,:,:]).reshape((-1,1,predictionLength), order="F")
    ...
    perm_train_h = np.loadtxt('perm_' + str(Line) + '.txt').tolist()
    listp = perm_train_h
    listb = []
    for i in listp:
        i =int(i)
...
#    perm_train_h = random.sample(range(0,perm_size),perm_size)
    
    #lr
    trainTripsId = perm_train_h[0:512]
    testTripsId = perm_train_h[-64::]
    t_h = historyTrip_11[:,trainTripsId]
    t_r = testTrip_11
    t_u = np.mean(t_h,1)
...
#    inter_num = np.size(data_inter,1)  #the numnber of inter
    ...
    num_station = len(select_station)
    num_inter = np.size(data_inter,1)   #除站点外的插值点数
    ...
    if bound == 0:
        ...
        N_SELECT = math.ceil(num_inter/3)
        
#    N_SELECT = num_station
#    N_SELECT = 72      #14
#    N_SELECT = 100     #110
#    N_SELECT = 70       #2
#    N_SELECT = 90       #421
#    N_SELECT = 35     #110    bound
#    N_SELECT = 140     #563
...
#    optimizer_rl = torch.optim.Adam(net.parameters(), lr=0.0001)
...
#        print(st_time)
        print('epoch:',epoch)
        #image数据循环
        trainTripNum = random.sample(range(1,trainTripNum_),200)
        
#        for j in range(trainTripNum): 
        for j in trainTripNum:           
...
#            print(j)
            ...
            state_a_tensor = torch.FloatTensor([])
            state_b_tensor = torch.FloatTensor([])
...
#                a = np.argmax(action_prob.cpu().data.numpy(), axis=2)    #1*1*8 int
                ...
                for i in range(num_inter):
                    ...
                    a = np.random.choice(action_list,  p=np.array(net(s_a,s_b,num_inter,N_SELECT,num_feature)[0].cpu().data.numpy())[i])
#                    a = np.random.choice(action_list, p = net(s_a,s_b)[0].cpu().data.numpy())
#                   a = np.random.choice(action_list, p = p )
                    p_test=np.array(net(s_a,s_b,num_inter,N_SELECT,num_feature)[0].cpu().data.numpy())
...
#                test = s_b.cpu().data
#                test = np.where(test[0,:,0]==1)
#                test = np.array(test).tolist()[0]
#                t = len(test)
                
                s_a2 = trans(s_a1, s_b,N_SELECT,num_feature)        #根据执行的动作更新选取的局部特征

                #reward
#                reward = cal_reward(data_all,target_all,select_station,select_inter,s_b.cpu().data,meanSq_target,busAllMaxTime)     #奖励为 lstm输出 
#                reward = cal_reward_lr(t_h,t_r,t_u,select_station,select_inter,s_b.cpu().data)                                          #奖励为 LR预测整个到站时间的MAE 
#                reward = cal_reward_lr_single(t_h,t_r,t_u,select_station,select_inter,s_b.cpu().data) 
                reward =  cal_reward_lr_single_station(t_h,t_r,t_u,select_station,select_inter,s_b.cpu().data,stationFlag_test)
#                reward =  cal_reward_lr_station(t_h,t_r,t_u,select_station,select_inter,s_b.cpu().data,stationFlag_test)                             #奖励为 LR由首站预测后多个站的平均误差
...
#                print(reward)x
                ...
                reward_tensor = torch.cat(([reward_tensor, torch.FloatTensor([[reward]])]))
                state_a_tensor = torch.cat((state_a_tensor,s_a.cpu()))
                state_b_tensor = torch.cat((state_b_tensor,s_b.cpu()))
                action_tensor = torch.cat((action_tensor, action))

                s_a = torch.cat((s_a1, s_a2), 1).cuda()    #更新sa
                
                s_en_time = time.time()
#                print('step_epoch:',step_num+1,'time:',s_en_time-s_st_time)

        
            j_en_time = time.time()
#            print('j_epoch:',j+1,'time:',j_en_time-j_st_time)
...
#            print(dis_reward)
            ...
            action = net(state_a_tensor.cuda(),state_b_tensor.cuda(),num_inter,N_SELECT,num_feature)
            action_tensor = Variable(action_tensor, requires_grad=True).cuda()
            dis_reward = Variable(torch.FloatTensor([dis_reward])).cuda()
        
            log_lik = -action_tensor.cpu().view((-1,num_inter,2)) * torch.log(action.view((-1,num_inter,2)).cpu())
#            print(log_lik)
            test_reward_list.append(log_lik.data)

            log_lik_adv = torch.FloatTensor([])
            for i in range(step):
                log_lik_adv = torch.cat((log_lik_adv,log_lik[i,:,:]*dis_reward[0,i,0]),dim = 0)
                
                
            
            log_lik_adv = log_lik_adv.view((1,step,num_inter,2))
#            print(log_lik_adepoch_listv)
            loss = torch.sum(log_lik_adv,1)
#            print(loss)
...
#            print(loss)
        ...
        en_time = time.time()
        print('epoch:',epoch,'train_time:',en_time-st_time,'loss',loss.data)
        

        test_reward_list.append(dis_reward.cpu().data.numpy())

        if (epoch+1) % 100 == 0: 
#            torch.save(net, 'params110_PGmodel_'+ str(epoch) + '.pkl')                     #保存奖励为 lstm输出 的网络
#            torch.save(net, 'params110_PGmodel_Lr_'+ str(epoch) + '.pkl')                  #保存奖励为 LR预测整个到站时间的MAE 的网络
#            torch.save(net, 'params' + str(Line) + '_PGmodel_LrSingle_'+ str(epoch) + '.pkl')            #保存奖励为 LR由首站预测后多个站的平均误差 的网络
#            torch.save(net, 'params' + str(Line) + '_PGmodel_LrSingle_'+ str(epoch) + '_bound' +'.pkl')            #保存奖励为 LR由首站预测后多个站的平均误差 并增加移动区间 的网络
#            torch.save(net, 'params' + str(Line) + '_PGmodel_LrSingle_'+ str(epoch) + '_n' + '.pkl')
#            torch.save(net, 'params' + str(Line) + '_PGmodel_LrSingle_'+ str(epoch) + '_bound' + '_n' + '_station'+ '.pkl')  #_n new
#            torch.save(net, 'params' + str(Line) + '_PGmodel_LrSingle_'+ str(epoch) + '_n' + '_station'+'N_select=station'+ '.pkl')
            torch.save(net, 'params' + str(Line) + '_PGmodel_LrSingle_'+ str(epoch) + '_n' + 'forpaper' + '.pkl')

            s_inter = np.where(s_b.cpu().data.numpy()[0,:,0] == 1)[0]
...
#            s_inter = np.where(s_b[0,:,0] == 1)
#            s_inter = np.array(s_inter).tolist()[0]
#            select_inter_s_b = np.array(select_ixnter).reshape(-1,1)
#            select_station_s_b = np.array(select_station).reshape(-1,1)
#            
#            s_inter_ = select_inter_s_b[s_inter]
#            aa = np.vstack((select_station_s_b,s_inter_))
#            s_index = np.sort(aa,axis=0).tolist()
            
# ===================================================================================           

  
#            np.savetxt('station_pg_select_110_' + str(epoch) + '.txt', s_inter) 
#            np.savetxt('station_pgLr_select_ ' + str(Line) + '_' + str(epoch) + '.txt', s_inter) 
#            np.savetxt('station_pgLrSingle_select_110_' + str(epoch) + '.txt', s_inter) 
            
#            s_inter = [str(i) for i in s_inter]                                        #保存每个epoch最后输出的是s_b为字符型，非浮点型数据
#            fh = open('STR_station_pg_select_110.txt', 'a', encoding='utf-8')
#            fh = open('STR_station_pgLr_select_110.txt', 'a', encoding='utf-8')
#            fh = open('STR_station_pgLrSingle_select_110.txt', 'a', encoding='utf-8')
#            for m in range(len(s_inter)):
#                fh.write(s_inter[m])
#                fh.write('\t')
#            fh.write('\r\n')    
#            fh.close()
...
#==================================测试 LR 输出结果====================================================================================            
            (ave1_list,ave2_list,step_list) = testModel_PolicyGradient_lr.testModel_PolicyGradient(net,trainTripNum_,data_inter,num_inter,num_feature,N_SELECT,stationFlag_test,historyTrip_11,testTrip_11,testTrip_21,Line)
            ave1_min = min(ave1_list)
            ave2_min = min(ave2_list)        
            step1_min = ave1_list.index(ave1_min)
            step2_min = ave2_list.index(ave2_min)
            step1_min = step_list[step1_min]
            step2_min = step_list[step2_min]
            print('Saving......')
            print('Epoch:', epoch)
            print('    aveAbsE_min_test(all): ', ave1_min)
            print('    aveAbsE_min_test(station): ', ave2_min)  
            print('    aveAbsE_min_step(all): ', step1_min)
            print('    aveAbsE_min_step(station): ', step2_min)
            print('\n')
            
            ave_min_list.append(ave2_min)
            epoch_list.append(epoch)
#======================================================================================================================            
            
            testen_time = time.time()
#            print('epoch:',epoch+1,'test_time:',testen_time - en_time,'loss',loss.data)


#    plt.plot(loss_list)
    plt.plot(epoch_list, ave_min_list)  
...
#    station_fanal_index = station_fanal(s_b,select_inter,select_station)
    
#    torch.save(net, 'params110_PGmodel'+ str(epoch) + '.pkl')
#    s_inter = np.where(s_b[0,:,0] == 1)
#    np.savetxt("station_pg_select_110.txt",s_inter) 
    
#    torch.load('rlmodel.pkl')
